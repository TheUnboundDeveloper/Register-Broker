...
| `nct6687d.pwm.0`–`.7` (labels `Fan {i} PWM`) | Super-I/O NCT668x EC | EC `0x160 + i` (single duty byte) | `raw/255·100` → % (`NctPwmPercent`) | **Read-only telemetry** — the duty the chip is currently driving each fan header at. NO write path exists (reading cannot change a fan). 🟡 built + selftest-gated; live HW validation pending. |
