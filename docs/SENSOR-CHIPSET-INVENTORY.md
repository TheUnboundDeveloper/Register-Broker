...
on MSI boards via USB-HID Mystic Light (`AllowHidRgb`, on by default; set `false` to opt out); the
NCT6687 EC 12V-header path is wired but inert pending validation. See `RGB-BOARD-BRINGUP.md` to add a
board's zones.
